import numpy as np


class ReplayBuffer:
    def __init__(self, max_size=1e6):
        self.storage = []
        self.max_size = max_size
        self.ptr = 0

    def add(self, data):
        if len(self.storage) == self.max_size:
            self.storage[int(self.ptr)] = data
            self.ptr = (self.ptr + 1) % self.max_size
        else:
            self.storage.append(data)

    def sample(self, batch_size):
        ind = np.random.randint(0, len(self.storage), size=int(batch_size))
        x, y, u, r = [], [], [], []

        for i in ind:
            X, Y, U, R = self.storage[i]
            x.append(np.array(X, copy=False))
            y.append(np.array(Y, copy=False))
            u.append(np.array(U, copy=False))
            r.append(np.array(R, copy=False))
        return np.array(x), np.array(y), np.array(u), np.array(r).reshape(-1, 1)


class ReplayBuffer2:
    def __init__(self, max_size=6e4):
        self.storage = []
        self.max_szie = max_size
        self.ptr = 0

    def add(self, data):
        if len(self.storage)==self.max_szie:
            self.storage[int(self.ptr)] = data
            self.ptr = (self.ptr + 1) % self.max_szie
        else:
            self.storage.append(data)

    def sample(self, batch_size):
        ind = np.random.randint(0, len(self.storage), size=int(batch_size))
        s, s_, a, r = [], [], [], []
        for i in ind:
            S, S_, A, R = self.storage[i]
            s.append(np.array(S, copy=False))
            s_.append(np.array(S_, copy=False))
            a.append(np.array(A, copy=False))
            r.append(np.array(R, copy=False))
        return np.array(s), np.array(s_), np.array(a), np.array(r).reshape(-1, 1)