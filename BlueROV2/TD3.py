...
class TD3(object):
    ...
    def train(self, replay_buffer, iterations, batch_size=100, discount=0.99, tau=0.005, policy_noise=0.2,
        ...
        for it in range(iterations):
            x, y, u, r = replay_buffer.sample(batch_size)
    ...
    def save(self, directory_a, directory_c):
        torch.save(self.actor.state_dict(), directory_a + 'model.pt')
        torch.save(self.critic.state_dict(), directory_c + 'model.pt')

    def load(self, model_path1, model_path2):
        self.actor.load_state_dict(torch.load(model_path1, map_location=lambda storage, loc: storage))
        self.actor.eval()
        self.critic.load_state_dict(torch.load(model_path2, map_location=lambda storage, loc: storage))
        self.critic.eval()
